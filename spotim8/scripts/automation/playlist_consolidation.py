...
from spotim8.features.genres import get_all_split_genres, SPLIT_GENRES
...
def consolidate_old_monthly_playlists(sp: spotipy.Spotify, keep_last_n_months: int = 3) -> None:
    ...
    if sync.ENABLE_MONTHLY:
        playlist_types["monthly"] = sync.PREFIX_MONTHLY
    if sync.ENABLE_MOST_PLAYED:
        playlist_types["most_played"] = sync.PREFIX_MOST_PLAYED
    ...
    if sync.ENABLE_DISCOVERY:
        playlist_types["discovery"] = sync.PREFIX_DISCOVERY
                ...
                for mon_abbr in sync.MONTH_NAMES.values():
                    if playlist_name.startswith(f"{monthly_pattern}{mon_abbr}"):
                        ...
                        if remaining.isdigit():
                            ...
                            month_num = None
                            for num, abbr in sync.MONTH_NAMES.items():
                                if abbr == mon_abbr:
                                    month_num = num
                                    break
    ...
    try:
        ...
        sync.log(f"  ⚠️  Could not load liked songs data: {e}")
    ...
    if history_df is not None and not history_df.empty:
        try:
            ...
            sync.log(f"  ⚠️  Could not process streaming history for consolidation: {e}")
        ...
        if year in year_to_tracks_history:
            for playlist_type in ["most_played", "discovery"]:  # Top and Discovery kept for yearly
                if playlist_type in year_to_tracks_history[year]:
                    ...
                    if (playlist_type == "most_played" and sync.ENABLE_MOST_PLAYED) or \
                       (playlist_type == "discovery" and sync.ENABLE_DISCOVERY):
                        # Check if yearly playlist exists
                        yearly_name = format_playlist_name(sync.YEARLY_NAME_TEMPLATE, year=year_short, playlist_type=playlist_type)
    ...
    if not years_to_consolidate:
        sync.log("  No old years need consolidation (all already consolidated)")
        return
    
    sync.log(f"  Found {len(years_to_consolidate)} year(s) to consolidate: {sorted(years_to_consolidate)}")
    ...
    try:
        ...
        sync.log(f"  ⚠️  Could not load genre data: {e}")
        sync.log(f"  Will create main playlists only (no genre splits)")
        ...
        for playlist_type, prefix in playlist_types.items():
            ...
            if year in monthly_playlists and playlist_type in monthly_playlists[year]:
                for monthly_name, monthly_id in monthly_playlists[year][playlist_type]:
                    tracks = sync.get_playlist_tracks(sp, monthly_id)
                    ...
                    sync.log(f"    - {monthly_name}: {len(tracks)} tracks")
                ...
                if playlist_type == "monthly" and year in year_to_tracks:
                    ...
                    for track in year_to_tracks[year]:
                        if track not in all_tracks_set:
                            all_tracks_list.append(track)
                            all_tracks_set.add(track)
                    sync.log(f"    - Using liked songs data for {playlist_type}: {len(year_to_tracks[year])} tracks")
                    ...
                    if year in year_to_tracks_history and playlist_type in year_to_tracks_history[year]:
                        ...
                        for track in year_to_tracks_history[year][playlist_type]:
                            if track not in all_tracks_set:
                                all_tracks_list.append(track)
                                all_tracks_set.add(track)
                        sync.log(f"    - Using streaming history for {playlist_type}: {len(year_to_tracks_history[year][playlist_type])} tracks")
                        ...
                        if not year_data.empty:
                            ...
                            if track_col:
                                ...
                                if playlist_type == "most_played":
                                    ...
                                    sync.log(f"    - Re-sorted {playlist_type} tracks by play count")
                                # Discovery tracks are already sorted by first play time (most recent first)
                                # from get_discovery_tracks, so we keep that order
                    except Exception as e:
                        sync.log(f"    ⚠️  Could not re-sort tracks by play count: {e}")
                        # Continue with existing order if sorting fails
            
            if not all_tracks_list:
                sync.log(f"    ⚠️  No tracks found for {year} ({playlist_type}), skipping")
                continue
            ...
            if playlist_type == "monthly":
                ...
                playlist_configs = [
                    ...
                    (format_playlist_name(sync.GENRE_YEARLY_TEMPLATE, genre="HipHop", year=year_short, playlist_type="genre_monthly"), "Hip Hop tracks", "HipHop"),
                    (format_playlist_name(sync.GENRE_YEARLY_TEMPLATE, genre="Dance", year=year_short, playlist_type="genre_monthly"), "Dance tracks", "Dance"),
                    (format_playlist_name(sync.GENRE_YEARLY_TEMPLATE, genre="Other", year=year_short, playlist_type="genre_monthly"), "Other tracks", "Other"),
                ]
            else:
                # For other types, create single yearly playlist
                # Use yearly template format (no month) for yearly playlists
                # The templates default to monthly format, so we use sync.YEARLY_NAME_TEMPLATE as base
            ...
            if not filtered_tracks:
                sync.log(f"    ⚠️  No {genre_filter or 'all'} tracks for {year}, skipping {playlist_name}")
                continue
            ...
            if playlist_name in existing:
                ...
                if year not in monthly_playlists:
                    sync.log(f"  {playlist_name}: already consolidated (skipping check)")
                    continue
                ...
                if to_add:
                    ...
                    sync.log(f"  {playlist_name}: +{len(to_add)} tracks (total: {len(filtered_tracks)}; manually added tracks preserved)")
                    ...
                    sync.log(f"  {playlist_name}: already up to date ({len(filtered_tracks)} tracks)")
                ...
                if playlist_name in existing:
                    sync.log(f"  ⚠️  Playlist {playlist_name} already exists, skipping creation")
                    continue
                ...
                for chunk in sync._chunked(filtered_tracks, 50):
                    sync.api_call(sp.playlist_add_items, pid, chunk)
                # Update description with genre tags
                sync._update_playlist_description_with_genres(sp, user_id, pid, filtered_tracks)
                sync.log(f"  {playlist_name}: created with {len(filtered_tracks)} tracks")
        
            # Delete old monthly playlists if they existed
            if year in monthly_playlists and playlist_type in monthly_playlists[year]:
                for monthly_name, monthly_id in monthly_playlists[year][playlist_type]:
                    try:
                        sync.api_call(sp.user_playlist_unfollow, user_id, monthly_id)
                        # Invalidate cache since we deleted a playlist
                        sync._invalidate_playlist_cache()
                        sync.log(f"    ✓ Deleted {monthly_name}")
                    except Exception as e:
                        sync.log(f"    ⚠️  Failed to delete {monthly_name}: {e}")
        
        sync.log(f"  ✅ Consolidated {year} into yearly playlists for all types")
...
def delete_old_monthly_playlists(sp: spotipy.Spotify) -> None:
    ...
    for genre in SPLIT_GENRES:
        genre_patterns.append(f"{genre}{sync.PREFIX_GENRE_MONTHLY}")
    ...
    if not playlists_to_delete:
        sync.log("  No old genre-split monthly playlists to delete")
        return
    
    sync.log(f"  Found {len(playlists_to_delete)} old genre-split monthly playlists to delete")
    ...
    for playlist_name, playlist_id in playlists_to_delete:
        try:
            sync.api_call(sp.user_playlist_unfollow, user_id, playlist_id)
            # Invalidate cache since we deleted a playlist
            sync._invalidate_playlist_cache()
            sync.log(f"    ✓ Deleted {playlist_name}")
        except Exception as e:
            sync.log(f"    ⚠️  Failed to delete {playlist_name}: {e}")
    
    sync.log(f"  ✅ Deleted {len(playlists_to_delete)} old genre-split monthly playlists")
...
def delete_duplicate_playlists(sp: spotipy.Spotify) -> None:
    ...
    existing = sync.get_existing_playlists(sp, force_refresh=True)
    user = sync.get_user_info(sp)
    user_id = user["id"]
    
    if not existing:
        sync.log("  ℹ️  No playlists found")
        return
    
    sync.log(f"  Checking {len(existing)} playlist(s) for duplicates...")
    ...
    if len(existing) > max_playlists:
        sync.log(f"  ⚠️  Too many playlists ({len(existing):,}) - skipping duplicate detection")
        sync.log(f"      Set MAX_PLAYLISTS_FOR_DUPLICATE_CHECK env var to override (current limit: {max_playlists})")
        return
    ...
    for name, playlist_id in existing.items():
        try:
            tracks = sync.get_playlist_tracks(sp, playlist_id, force_refresh=False)  # Use cache if available
            ...
            checked += 1
            if checked % 50 == 0:
                sync.log(f"  Progress: checked {checked}/{len(existing)} playlists...")
        except Exception as e:
            sync.log(f"  ⚠️  Could not read tracks from '{name}': {e}")
            continue
    ...
    for track_set, playlists in playlist_track_sets.items():
        if len(playlists) > 1 and len(track_set) > 0:  # Only consider non-empty playlists
            ...
            sync.log(f"  🔍 Found {len(playlists)} duplicate playlist(s) with {len(track_set)} tracks:")
            sync.log(f"     ✅ Keeping: '{keep_playlist[0]}'")
            
            for dup_name, dup_id in duplicates:
                try:
                    sync.api_call(sp.user_playlist_unfollow, user_id, dup_id)
                    sync.log(f"     🗑️  Deleted: '{dup_name}'")
                    deleted_count += 1
                    # Remove from cache
                    sync._invalidate_playlist_cache()
                    ...
                    sync.log(f"     ⚠️  Failed to delete '{dup_name}': {e}")
    
    if deleted_count > 0:
        sync.log(f"  ✅ Deleted {deleted_count} duplicate playlist(s)")
    else:
        sync.log("  ℹ️  No duplicate playlists found")
