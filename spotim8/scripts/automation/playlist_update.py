...
from spotim8.features.genres import get_all_split_genres, get_all_broad_genres, SPLIT_GENRES
...
def update_monthly_playlists(sp: spotipy.Spotify, keep_last_n_months: int = 3) -> dict:
    ...
    enabled_types = []
    if sync.ENABLE_MONTHLY:
        enabled_types.append("Finds (monthly)")
    if sync.ENABLE_MOST_PLAYED:
    ...
    if sync.ENABLE_DISCOVERY:
        enabled_types.append("Dscvr (yearly only)")
    
    if enabled_types:
        sync.log(f"  Enabled playlist types: {', '.join(enabled_types)}")
        sync.log(f"  📌 Note: Top/Dscvr are created as yearly playlists only (no monthly). Vbz/Rpt removed.")
    else:
        sync.log("  ⚠️  No playlist types enabled - check .env file")
        return {}
    ...
    from spotim8.analysis.streaming_history import load_streaming_history
    history_df = load_streaming_history(sync.DATA_DIR)
            ...
            try:
                ...
                if pd.notna(latest_history):
                    ...
                    if days_behind > 30:
                        ...
                        sync.log(f"  ⚠️  Streaming history is {days_behind} days old (latest: {latest_str})")
                        sync.log(f"      Recent months may be incomplete. Export new data for up-to-date playlists.")
            except Exception:
                pass  # Skip freshness check if there's an error
        
        sync.log(f"  Loaded streaming history: {len(history_df):,} records")
    else:
        sync.log("  ⚠️  No streaming history found - Discovery playlists will be empty")
        sync.log("      Export streaming history data to enable these playlist types")
    
    # Load liked songs data for "Finds" playlists (API data only - never uses streaming history)
    playlist_tracks_path = sync.DATA_DIR / "playlist_tracks.parquet"
    all_month_to_tracks = {}
    
    if playlist_tracks_path.exists():
        library = pd.read_parquet(playlist_tracks_path)
        liked = library[library["playlist_id"].astype(str) == sync.LIKED_SONGS_PLAYLIST_ID].copy()
        ...
        if not liked.empty:
            ...
            if added_col:
                ...
                sync.log(f"  Loaded liked songs (API data) for 'Finds' playlists: {len(all_month_to_tracks)} month(s)")
        else:
            sync.log("  ⚠️  No liked songs found in library data - 'Finds' playlists will be empty")
    else:
        sync.log("  ⚠️  Library data not found - 'Finds' playlists will be empty (run full sync first)")
    ...
    if all_months:
        ...
        if older_months:
            sync.log(f"📅 Keeping {len(recent_months)} recent months as monthly playlists: {', '.join(recent_months)}")
            sync.log(f"📦 {len(older_months)} older months will be consolidated into yearly playlists")
            if finds_months:
                finds_recent = [m for m in recent_months if m in finds_months]
                sync.log(f"   📌 'Finds' playlists will use API data (liked songs) for {len(finds_recent)} month(s)")
    else:
        recent_months = []
    
    if not recent_months:
        sync.log("No months to process")
        return {}
    
    sync.log(f"Processing {len(recent_months)} month(s) for all playlist types...")
    
    # Get existing playlists (cached)
    existing = sync.get_existing_playlists(sp)
    user = sync.get_user_info(sp)
    user_id = user["id"]
    ...
    if sync.ENABLE_MONTHLY:
        playlist_configs.append((
            "monthly", sync.MONTHLY_NAME_TEMPLATE, "Liked songs", 
    ...
    if not playlist_configs:
        sync.log("⚠️  All playlist types are disabled in .env file. No playlists will be created.")
        return {}
    ...
    for month in sorted(recent_months):
        ...
        for playlist_type, template, description, get_tracks_fn in playlist_configs:
            ...
            if name in existing:
                pid = existing[name]
                already = sync.get_playlist_tracks(sp, pid)
                to_add = [u for u in track_uris if u not in already]
                
                if to_add:
                    for chunk in sync._chunked(to_add, 50):
                        sync.api_call(sp.playlist_add_items, pid, chunk)
                    if pid in sync._playlist_tracks_cache:
                        del sync._playlist_tracks_cache[pid]
                    sync.log(f"  {name}: +{len(to_add)} tracks ({len(track_uris)} total)")
                    # Update description with genre tags
                    sync._update_playlist_description_with_genres(sp, user_id, pid, track_uris)
                else:
                    sync.log(f"  {name}: up to date ({len(track_uris)} tracks)")
                ...
                # Create playlist
                sync.verbose_log(f"Creating new playlist '{name}' for {month} (type={playlist_type})...")
                pl = sync.api_call(
                ...
                pid = pl["id"]
                sync.verbose_log(f"  Created playlist '{name}' with id {pid}")
                
                # Add tracks
                sync.verbose_log(f"  Adding {len(track_uris)} tracks in chunks...")
                chunk_count = 0
                for chunk in sync._chunked(track_uris, 50):
                    chunk_count += 1
                    sync.verbose_log(f"    Adding chunk {chunk_count} ({len(chunk)} tracks)...")
                    sync.api_call(sp.playlist_add_items, pid, chunk)
                
                # Update description with genre tags
                sync._update_playlist_description_with_genres(sp, user_id, pid, track_uris)
                
                sync._invalidate_playlist_cache()
                sync.verbose_log(f"  Invalidated playlist cache after creating new playlist")
                sync.log(f"  {name}: created with {len(track_uris)} tracks")
...
def update_genre_split_playlists(sp: spotipy.Spotify, month_to_tracks: dict) -> None:
    ...
    # Load genre data
    track_artists = pd.read_parquet(sync.DATA_DIR / "track_artists.parquet")
    artists = pd.read_parquet(sync.DATA_DIR / "artists.parquet")
    ...
    track_to_genres = {}
    tracks_df = pd.read_parquet(sync.DATA_DIR / "tracks.parquet")
    if "genres" in tracks_df.columns:
        # Use stored track genres
        for _, track_row in tracks_df.iterrows():
            track_id = track_row["track_id"]
            uri = f"spotify:track:{track_id}"
            stored_genres = sync._parse_genres(track_row.get("genres"))
            if stored_genres:
    ...
    for track_id in track_ids:
        ...
        all_track_genres = sync._get_all_track_genres(track_id, track_artists, artist_genres_map)
    ...
    for month, uris in sorted(month_to_tracks.items()):
        for genre in SPLIT_GENRES:
            ...
            name = format_playlist_name(sync.GENRE_MONTHLY_TEMPLATE, month, genre, playlist_type="genre_monthly")
            ...
            if name in existing:
                ...
                if to_add:
                    sync.verbose_log(f"Adding {len(to_add)} tracks to playlist '{name}' (playlist_id={pid})")
                    chunk_count = 0
                    for chunk in sync._chunked(to_add, 50):
                        chunk_count += 1
                        sync.verbose_log(f"  Adding chunk {chunk_count} ({len(chunk)} tracks)...")
                    ...
                    if pid in sync._playlist_tracks_cache:
                        del sync._playlist_tracks_cache[pid]
                        sync.verbose_log(f"  Invalidated cache for playlist {pid}")
                    sync.log(f"  {name}: +{len(to_add)} tracks (manually added tracks preserved)")
                    # Update description with genre tags (use all tracks in playlist)
                    sync._update_playlist_description_with_genres(sp, user_id, pid, None)
            else:
                pl = sync.api_call(
                    sp.user_playlist_create,
                    user_id,
                ...
                for chunk in sync._chunked(genre_uris, 50):
                    sync.api_call(sp.playlist_add_items, pid, chunk)
                # Update description with genre tags
                sync._update_playlist_description_with_genres(sp, user_id, pid, genre_uris)
                # Invalidate playlist cache since we created a new playlist
                sync._invalidate_playlist_cache()
                sync.log(f"  {name}: created with {len(genre_uris)} tracks")
...
def _remove_genre_from_track(tracks_df: pd.DataFrame, track_uri: str, genre_to_remove: str) -> bool:
    ...
    current_genres = sync._parse_genres(tracks_df.at[track_idx, "genres"])
...
def update_master_genre_playlists(sp: spotipy.Spotify) -> None:
    ...
    # Load data
    library = pd.read_parquet(sync.DATA_DIR / "playlist_tracks.parquet")
    track_artists = pd.read_parquet(sync.DATA_DIR / "track_artists.parquet")
    artists = pd.read_parquet(sync.DATA_DIR / "artists.parquet")
    
    # Get liked songs
    liked = library[library["playlist_id"].astype(str) == sync.LIKED_SONGS_PLAYLIST_ID]
    ...
    # Select top genres
    selected = [g for g, n in genre_counts.most_common(sync.MAX_GENRE_PLAYLISTS) 
                if n >= sync.MIN_TRACKS_FOR_GENRE]
    
    sync.log(f"  Found {len(selected)} genre(s) with >= {sync.MIN_TRACKS_FOR_GENRE} tracks")
    ...
    if "playlist_id" in library.columns and "track_uri" in library.columns:
        for genre in selected:
            name = format_playlist_name(sync.GENRE_NAME_TEMPLATE, genre=genre, playlist_type="genre_master")
            if name in existing:
                pid = existing[name]
    ...
    for genre in selected:
        ...
        if name in existing:
            pid = existing[name]
            # Get existing tracks (includes both auto-added and manually added tracks)
            already_in_playlist = sync.get_playlist_tracks(sp, pid)
            ...
            if tracks_to_remove:
                sync.log(f"  {name}: Removing {len(tracks_to_remove)} track(s) that don't match genre...")
                # Remove tracks by URI (Spotify API supports this)
                for chunk in sync._chunked(tracks_to_remove, 50):
                    sync.api_call(sp.playlist_remove_all_occurrences_of_items, pid, chunk)
                
                # Invalidate cache
                if pid in sync._playlist_tracks_cache:
                    del sync._playlist_tracks_cache[pid]
                # Re-fetch to get updated list
                already_in_playlist = sync.get_playlist_tracks(sp, pid, force_refresh=True)
            ...
            if pid in previous_playlist_tracks:
                ...
                if removed_tracks:
                    ...
                    if tracks_to_remove_genre:
                        sync.log(f"  {name}: Detected {len(tracks_to_remove_genre)} manually removed track(s), removing genre tags...")
            ...
            sync.verbose_log(f"Creating new playlist '{name}' for genre '{genre}'...")
            pl = sync.api_call(
                sp.user_playlist_create,
                user_id,
                name,
            ...
            pid = pl["id"]
            sync.verbose_log(f"  Created playlist '{name}' with id {pid}, adding {len(uris_should_be_in_playlist)} tracks...")

            chunk_count = 0
            for chunk in sync._chunked(list(uris_should_be_in_playlist), 50):
                chunk_count += 1
                sync.verbose_log(f"  Adding chunk {chunk_count} ({len(chunk)} tracks) to new playlist...")
                sync.api_call(sp.playlist_add_items, pid, chunk)
            # Update description with genre tags
            sync._update_playlist_description_with_genres(sp, user_id, pid, list(uris_should_be_in_playlist))
            # Invalidate playlist cache since we created a new playlist
            sync._invalidate_playlist_cache()
            sync.log(f"  {name}: created with {len(uris_should_be_in_playlist)} tracks")
    
    # Save tracks_df if it was modified
    if tracks_modified:
        tracks_path = sync.DATA_DIR / "tracks.parquet"
        try:
            tracks_df.to_parquet(tracks_path, index=False, engine='pyarrow')
        except Exception:
            tracks_df.to_parquet(tracks_path, index=False)
        sync.log(f"  ✅ Updated track genres after removals")
