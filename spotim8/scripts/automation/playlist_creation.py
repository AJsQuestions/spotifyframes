...
def create_or_update_playlist(
    ...
    if playlist_name in existing_playlists:
        ...
        if to_add:
            ...
            sync.log(f"  {playlist_name}: +{len(to_add)} tracks (total: {len(track_uris)})")
            # Update description with genre tags
            sync._update_playlist_description_with_genres(sp, user_id, pid, track_uris)
        else:
            sync.log(f"  {playlist_name}: up to date ({len(track_uris)} tracks)")
        ...
        sync.log(f"  {playlist_name}: created with {len(track_uris)} tracks")
        return pid
