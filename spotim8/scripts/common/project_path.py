...
def get_project_root(current_file: str) -> Path:
    ...
    # Count how many "spotim8" directories we need to go up
    parts = current_path.parts
    try:
        spotim8_index = parts.index("spotim8")
        # Go up to spotim8, then one more to project root
        return Path(*parts[:spotim8_index + 1]).parent
    except ValueError:
